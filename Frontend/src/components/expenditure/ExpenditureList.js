...
const ExpenditureList = () => {
  ...
  const fetchExpenditures = useCallback(async () => {
    try {
      ...
      if (projRes.data.project.user_id !== userRes.data.id) {
      ...
      setError("Failed to load expenditures or verify access.");
    } finally {
      setCheckingOwner(false);
      setLoading(false);
    }
  ...
  const handleDelete = async (expId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://localhost:8000/api/expenditures/${expId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setExpenditures((prev) => prev.filter((e) => e.id !== expId));
    } catch {
  ...
  return (
    <div className="container-fluid">
      <div className="row">
        {/* Sidebar is in Navigation; content offset by 2 cols on lg+ */}
        <div className="col-12 col-lg-10 offset-lg-2 py-4">
          <div className="d-flex justify-content-between align-items-center mb-3">
            <h3 className="mb-0">Expenditure Records</h3>
            <div>
